...
function stringMasLarga(str) {
  ...
  var maslarga = strsplit.reduce((a, b) => (b.length > a.length) ? b : a); return maslarga;
  }
...
function buscaDestruye(arreglo, num) {
  ...
  //
  // Tu código aca:
  arreglo = arreglo.filter(function(el){ return el !== num; });
   return arreglo;
  }
...
function sumarElTipo(arreglo) {
  ...
  // Tu código aca:
  return arreglo.reduce((a, d) => (a[d] ? a[d] += 1: a[d] = 1, a), {})
}
...
function mapear() {
  ...
  Array.prototype.mapear = function() {
    var newArr = []; 
    //for (let i = 0; i < this.length; i++) {
    this.forEach(function(elemento){
      newArr.push(this[elemento])
    }) 
    console.log(this[1])
     }  
  }
